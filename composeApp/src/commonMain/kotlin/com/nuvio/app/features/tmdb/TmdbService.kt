...
object TmdbService {
    ...
    /** Minimal metadata used to translate an ID-based Nuvio request into a title-based provider search. */
    suspend fun pluginMediaMetadata(tmdbId: String, mediaType: String): TmdbPluginMediaMetadata? {
        val apiKey = currentApiKey() ?: return null
        val id = tmdbId.trim().toIntOrNull() ?: return null
        val type = normalizeMediaType(mediaType)
        val endpoint = if (type == "tv") "tv/$id" else "movie/$id"
        val body = fetch<TmdbPluginMediaResponse>(endpoint = endpoint, apiKey = apiKey) ?: return null
        val title = (body.title ?: body.name)?.trim()?.takeIf(String::isNotBlank) ?: return null
        val date = body.releaseDate ?: body.firstAirDate
        return TmdbPluginMediaMetadata(title = title, year = date?.take(4)?.toIntOrNull())
    }
...
data class TmdbPluginMediaMetadata(
    val title: String,
    val year: Int?,
)

@Serializable
private data class TmdbPluginMediaResponse(
