...
internal const val CLOUDSTREAM_PLUGIN_CODE_PREFIX = "nuvio-cloudstream-v1:"
private val cloudstreamDescriptorJson = Json { ignoreUnknownKeys = true; encodeDefaults = true }
...
@Serializable
internal data class CloudstreamExtensionDescriptor(
...
internal fun CloudstreamExtensionDescriptor.toPluginCode(): String =
    CLOUDSTREAM_PLUGIN_CODE_PREFIX + cloudstreamDescriptorJson.encodeToString(CloudstreamExtensionDescriptor.serializer(), this)

internal fun parseCloudstreamPluginCode(code: String): CloudstreamExtensionDescriptor? {
    if (!code.startsWith(CLOUDSTREAM_PLUGIN_CODE_PREFIX)) return null
    return runCatching {
        cloudstreamDescriptorJson.decodeFromString(
            CloudstreamExtensionDescriptor.serializer(),
            code.removePrefix(CLOUDSTREAM_PLUGIN_CODE_PREFIX),
        )
    }.getOrNull()
}
